...
rc.keys "fi:*", (err, ms) ->
  ...
  console.log "migrating #{ms.length} sets of friend data"
  count = 0
  async.each(
    ms
    (counterkey, callback) ->
      ...
      rc.hgetall "fi:#{user}", (err, fdata) ->
        if err?
          console.log "error getting friend image for #{counterkey}, err: #{err}"
          process.exit(10)
        console.log "migrating friend image data keys from hash #{counterkey}"
        #insert friend image data into cassandra
        ...
        async.each(
          ...
          (fkey, callback) ->
            ...
            fkeys = fkey.split ":"
            console.log "inserting friend data into cassandra: #{user}, #{fkeys[0]}, #{fkeys[1]}, #{fdata[fkey]}"
            #insert keys into cassandra

            cdb.migrateInsertFriendImageData user, fkeys[0], fkeys[1],fdata[fkey], (err, result) ->
              if err?
