...
rc.keys "cm:*", (err, ms) ->
  ...
  async.each(
    ...
    (counterkey, callback) ->
      ...
      if splits.length is 4 or splits.length is 2
        ...
        rc.zrange counterkey, 0,  -1, (err, messages) ->
          if err?
            ...
            process.exit(10)
          console.log "migrating #{messages.length} messages from set #{counterkey}"
          #insert messages into cassandra
          async.each(
            messages
            (m, callback1) ->
              spot = "#{splits[1]}:#{splits[2]}"
              console.log "inserting #{++count} message into #{spot}"
              message = JSON.parse(m)
              message.moredata = "#{message.moredata}"
              cdb.insertMessageControlMessage spot, message, (err, result) ->
                if err
                  console.log "error inserting #{m} control message into #{spot}, err: #{err}"
                  process.exit(10)
                callback1()
            (err) ->
