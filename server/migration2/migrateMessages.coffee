    ...
    (counterkey, callback) ->
      ...
      if splits.length is 4 or splits.length is 2
        ...
        rc.zrange counterkey, 0,  -1, (err, messages) ->
          if err?
            console.log "error gettingg messages for #{counterkey}, err: #{err}"
            process.exit(10)
          ...
          async.each(
            messages
            (m, callback1) ->
              console.log "inserting #{++count} message for #{counterkey}"
              message = JSON.parse(m)

              cdb.migrateInsertMessage message, (err, result) ->
                if err?
                  console.log "error inserting #{m} message into #{counterkey}, err: #{err}"
                  process.exit(10)
                callback1()
            (err) ->
